...
- **Llamada en segundo plano (estilo WhatsApp)** — minimiza la llamada con el botón ↓ y sigue chateando o navegando por la app: un banner flotante verde con avatar, nombre y cronómetro indica que la llamada sigue activa (el audio continúa sonando); tócalo para volver a la pantalla de llamada o cuélgala desde el propio banner. El título de la pestaña también muestra «en llamada».
...
Espacios de temas (todos bajo `nexo/v1/`): `auth/<u>`, `profile/<u>` (nombre+foto), `presence/<u>`, `psub/<u>` (suscripción push), `freq/<para>/<de>`, `fresp/<para>/<de>`, `dm/<para>/<de>/<idMensaje>[/fragmento]`, `evt/<u>` (también: invitaciones de llamada grupal y señalización de cámara/pantalla), `group/<gid>` (descriptor), `ginv/<u>/<gid>` (invitación retenida), `gm/<gid>/<autor>/<id>[/fragmento]` (mensajes de grupo, stickers incluidos), `gm/<gid>/sys/call` (eventos de llamada grupal: unirse/salir/medios).
